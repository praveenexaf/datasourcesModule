...
export class AppComponent {
  ...
  openNodeConfigForm(parent: TreeNode) {
    ...
    this.nodeConfigForm = this.fb.group({
      ...
      limit: this.fb.control(this.activeNode['limit'], []),
      join: this.fb.array(
        this.activeNode['join'].map((x) => this.fb.control(x, [])),
        []
      ),
    });
    this.showAddNodeForm = false;
  ...
  addJoinCondition() {
    console.log('join :' + this.nodeConfigForm.get('join').value);

    this.nodeConfigForm
      .get('join')
      .value.push({ ...this.fb.control(this.newCondition) });
    console.log(this.nodeConfigForm);
  }
  deleteCondition(condition: joinCondition) {
    this.nodeConfigForm
      .get('join')
      .setValue(
        this.nodeConfigForm.get('join').value.filter((i) => i != condition)
      );
  }
